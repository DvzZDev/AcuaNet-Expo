import { Link, Stack } from "expo-router"
import { Image, Text, TouchableOpacity, View } from "react-native"
import { SafeAreaView } from "react-native-safe-area-context"
import { StatusBar } from "expo-status-bar"
import { TextInput } from "react-native-gesture-handler"
import { Checkbox } from "expo-checkbox"
import { useForm } from "@tanstack/react-form"
import { supabase } from "lib/supabase"

export default function SignIn() {
  const form = useForm({
    defaultValues: {
      name: "",
      lastName: "",
      email: "",
      password: "",
      confirmPassword: "",
      acceptTerms: false,
    },
    onSubmit: async ({ value }) => {
      // Validación adicional para contraseñas
      if (value.password !== value.confirmPassword) {
        alert("Las contraseñas no coinciden")
        return
      }

      if (!value.acceptTerms) {
        alert("Debes aceptar los términos y condiciones")
        return
      }

      const { data, error } = await supabase.auth.signUp({
        email: value.email,
        password: value.password,
      })

      if (error) {
        console.error("Error al registrar:", error.message)
        alert("Error al registrar: " + error.message)
        return
      }

      console.log("Usuario registrado:", data.user)
    },
  })

  return (
    <>
      <Stack.Screen
        options={{
          headerShown: false,
        }}
      />
      <StatusBar style="light" />
      <View className="flex-1">
        <SafeAreaView
          edges={["top"]}
          className="bg-[#16151a]"
        />

        <Image
          className="absolute right-0 top-0 z-10 ml-2"
          style={{ width: 550, height: 350 }}
          source={require("@assets/Star.png")}
        />

        <SafeAreaView
          className="flex-1 bg-[#14141c]"
          edges={["left", "right", "bottom"]}
        >
          <Image
            style={{ width: 250, height: 70 }}
            source={require("@assets/LogoHorizontalPng.png")}
            className="mx-auto mt-5"
          />
          <View className="mt-5 flex-col justify-center px-4 pb-7 pt-2">
            <Text className="mt-4 font-Inter-SemiBold text-4xl text-green-100">Crea tu cuenta</Text>
            <Text className="mt-4 font-Inter-Medium text-sm leading-relaxed text-green-200">
              Empieza creando una cuenta donde almacenaremos tus datos dentro de AcuaNet.
            </Text>
          </View>

          <View className="mt-10 h-full flex-col gap-2 rounded-t-3xl bg-green-100 p-5">
            <View className="flex w-full flex-row">
              <form.Field
                name="name"
                validators={{
                  onChange: ({ value }) => (!value ? "El nombre es requerido" : undefined),
                }}
              >
                {(field) => (
                  <View className="flex-1 pr-2">
                    <Text
                      className="font-Inter-SemiBold text-base text-green-900"
                      aria-label="Label for Name"
                      nativeID="labelName"
                    >
                      Nombre
                    </Text>
                    <TextInput
                      className="mt-2 h-12 w-full rounded-lg bg-[#14141c] px-4 font-Inter-Medium text-base text-green-100"
                      aria-label="input"
                      aria-labelledby="labelName"
                      value={field.state.value}
                      onChangeText={field.handleChange}
                      onBlur={field.handleBlur}
                    />
                    {field.state.meta.errors && (
                      <Text className="mt-1 text-sm text-red-500">{field.state.meta.errors[0]}</Text>
                    )}
                  </View>
                )}
              </form.Field>

              <form.Field
                name="lastName"
                validators={{
                  onChange: ({ value }) => (!value ? "Los apellidos son requeridos" : undefined),
                }}
              >
                {(field) => (
                  <View className="flex-1 pl-2">
                    <Text
                      className="font-Inter-SemiBold text-base text-green-900"
                      aria-label="Label for LastName"
                      nativeID="labelLastName"
                    >
                      Apellidos
                    </Text>
                    <TextInput
                      className="mt-2 h-12 w-full rounded-lg bg-[#14141c] px-4 font-Inter-Medium text-base text-green-100"
                      aria-label="input"
                      aria-labelledby="labelLastName"
                      value={field.state.value}
                      onChangeText={field.handleChange}
                      onBlur={field.handleBlur}
                    />
                    {field.state.meta.errors && (
                      <Text className="mt-1 text-sm text-red-500">{field.state.meta.errors[0]}</Text>
                    )}
                  </View>
                )}
              </form.Field>
            </View>

            <form.Field
              name="email"
              validators={{
                onChange: ({ value }) => {
                  if (!value) return "El email es requerido"
                  if (!/\S+@\S+\.\S+/.test(value)) return "Email inválido"
                  return undefined
                },
              }}
            >
              {(field) => (
                <View>
                  <Text
                    className="font-Inter-SemiBold text-base text-green-900"
                    aria-label="Label for Email"
                    nativeID="labelEmail"
                  >
                    Email
                  </Text>
                  <TextInput
                    className="mt-2 h-12 w-full rounded-lg bg-[#14141c] px-4 font-Inter-Medium text-base text-green-100"
                    aria-label="input"
                    aria-labelledby="labelEmail"
                    value={field.state.value}
                    onChangeText={field.handleChange}
                    onBlur={field.handleBlur}
                    keyboardType="email-address"
                    autoCapitalize="none"
                  />
                  {field.state.meta.errors && (
                    <Text className="mt-1 text-sm text-red-500">{field.state.meta.errors[0]}</Text>
                  )}
                </View>
              )}
            </form.Field>

            <form.Field
              name="password"
              validators={{
                onChange: ({ value }) => {
                  if (!value) return "La contraseña es requerida"
                  if (value.length < 6) return "La contraseña debe tener al menos 6 caracteres"
                  return undefined
                },
              }}
            >
              {(field) => (
                <View>
                  <Text
                    className="font-Inter-SemiBold text-base text-green-900"
                    aria-label="Label for Password"
                    nativeID="labelPassword"
                  >
                    Contraseña
                  </Text>
                  <TextInput
                    className="mt-2 h-12 w-full rounded-lg bg-[#14141c] px-4 font-Inter-Medium text-base text-green-100"
                    aria-label="input"
                    aria-labelledby="labelPassword"
                    secureTextEntry={true}
                    value={field.state.value}
                    onChangeText={field.handleChange}
                    onBlur={field.handleBlur}
                  />
                  {field.state.meta.errors && (
                    <Text className="mt-1 text-sm text-red-500">{field.state.meta.errors[0]}</Text>
                  )}
                </View>
              )}
            </form.Field>

            <form.Field
              name="confirmPassword"
              validators={{
                onChangeListenTo: ["password"],
                onChange: ({ value, fieldApi }) => {
                  if (!value) return "Confirma tu contraseña"
                  if (value !== fieldApi.form.getFieldValue("password")) {
                    return "Las contraseñas no coinciden"
                  }
                  return undefined
                },
              }}
            >
              {(field) => (
                <View>
                  <Text
                    className="font-Inter-SemiBold text-base text-green-900"
                    aria-label="Label for Confirm Password"
                    nativeID="labelConfirmPassword"
                  >
                    Repite la contraseña
                  </Text>
                  <TextInput
                    className="mt-2 h-12 w-full rounded-lg bg-[#14141c] px-4 font-Inter-Medium text-base text-green-100"
                    aria-label="input"
                    aria-labelledby="labelConfirmPassword"
                    secureTextEntry={true}
                    value={field.state.value}
                    onChangeText={field.handleChange}
                    onBlur={field.handleBlur}
                  />
                  {field.state.meta.errors && (
                    <Text className="mt-1 text-sm text-red-500">{field.state.meta.errors[0]}</Text>
                  )}
                </View>
              )}
            </form.Field>

            <form.Field
              name="acceptTerms"
              validators={{
                onChange: ({ value }) => (!value ? "Debes aceptar los términos y condiciones" : undefined),
              }}
            >
              {(field) => (
                <View>
                  <Text
                    className="font-Inter-SemiBold text-base text-green-900"
                    aria-label="Label for Terms"
                    nativeID="labelTerms"
                  >
                    Aceptas los terminos y condiciones{" "}
                    <Link
                      href={"https://acuanet.com/terms"}
                      className="text-green-900"
                    >
                      <Text>(Ver Aqui)</Text>
                    </Link>
                  </Text>
                  <Checkbox
                    className="mt-2 h-6 w-6 rounded-lg border-2 border-green-900 bg-[#14141c] p-1"
                    value={field.state.value}
                    onValueChange={field.handleChange}
                    color={field.state.value ? "#a5d5a8" : undefined}
                    aria-label="Checkbox for accepting terms and conditions"
                  />
                  {field.state.meta.errors && (
                    <Text className="mt-1 text-sm text-red-500">{field.state.meta.errors[0]}</Text>
                  )}
                </View>
              )}
            </form.Field>

            <form.Subscribe selector={(state) => [state.canSubmit, state.isSubmitting]}>
              {([canSubmit, isSubmitting]) => (
                <TouchableOpacity
                  onPress={form.handleSubmit}
                  disabled={!canSubmit}
                  className={`mt-5 h-fit w-full rounded-lg border-2 border-[#013204] p-5 ${
                    canSubmit ? "bg-[#a5d5a8]" : "bg-gray-400 opacity-50"
                  }`}
                >
                  <Text className="text-center font-Inter-SemiBold text-2xl text-green-950">
                    {isSubmitting ? "Registrando..." : "Registrarse"}
                  </Text>
                </TouchableOpacity>
              )}
            </form.Subscribe>
          </View>
        </SafeAreaView>
      </View>
    </>
  )
}
